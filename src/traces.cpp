...
) : cfg(config), bc(bincalc), db(db_handle),
    bins (bins_vector), bins_offset(bo_vector),
    rcv_sps(rcv), src_sps(src), x_sps(xrel) {}
    ...
    for (const auto& x_row : x_sps) {
        ...
        for (const auto& rcv_item : rcv_items) {
            ...
            auto bin_item = lower_bound(
                ...
                { return b.bin_id < value; }
            );
            if (bin_item != bins.end()) {
                update_bin_offset(bin_item->bin_id, src_index, offset);
                if (offset <= cfg.offset && ranges::find(cfg.src_indexes, src_index) != cfg.src_indexes.end())
                {
                    if (bin_item->offset < 1) {
                        bin_item->offset = cfg.offset;
                        bin_item->src_indexes = cfg.src_indexes;
                    }
                    bin_item->bin_count++;
                }
...
void Binning::update_bin_offset(int bin_id, int src_index, float offset)
{
    if (offset > cfg.offset_range[1]) return;

    {
        auto bin_offset_matches = ranges::equal_range(bins_offset, bin_id, {}, &BinOffsetStruct::bin_id);
        for (auto& bin_offset_item : bin_offset_matches)
        {
            if (bin_offset_item.offset >= offset && bin_offset_item.src_index == src_index) {
                bin_offset_item.bin_count++;
            }
        }
    }
}
